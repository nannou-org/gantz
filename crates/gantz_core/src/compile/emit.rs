...
//! is `if`, `begin`, `define`, `let` and `set!`. The VM runs
//! `Engine::new_base()`, which has no `cond` or `and` prelude.
//!
...
///
/// `define-values` is deliberately avoided. The steel-core 0.7 analysis pass
