...
pub(crate) fn flow_graph_with_extra(
    ...
    mask_header_seed_confs(&mut fg, &loops);
    Ok((fg, loops))
}

/// Clear each loop header's back-edge-only inputs (carried inputs with no external
/// initial source) from its flow conf, yielding the header's *seed* active-set.
///
/// On the entry (seed) pass a back-edge-only input has not been produced yet, so
/// the header must compile to the variant that treats it as inactive - e.g. the
/// pd+ accumulator entered via its hot inlet only (`{hot}`, cold cleared). Its
/// iteration variant (`{cold}`) is generated by conf injection and emitted by the
/// loop codegen. A counter, whose carried input has an external seed, clears
...
fn mask_header_seed_confs(fg: &mut FlowGraph, loops: &super::loops::LoopTable) {
    for info in loops.values() {
        ...
        for blk in fg.node_weights_mut() {
            for conf in blk.iter_mut().filter(|c| c.id == info.header) {
                for &i in &clear {
                    let _ = conf.conns.inputs.set(i, false);
                }
            }
        }
    }
...
/// The *iteration* (back-edge pass) [`NodeConf`] for each loop header across all
/// entrypoints: the header's flow conf with its inputs replaced by the carried
/// (back-edge) inputs, keeping its intrinsic outputs.
///
/// The loop codegen calls this variant on every iteration, but - unlike the seed
/// variant, which is a real flow block - it never appears as a flow conf. Callers
/// inject these into the node-conf collection so the variant's node fn (and, for a
/// `GraphNode` header, its active-set-monomorphized interior) is generated. For a
/// counter (carried input also externally seeded) the iteration conf equals the
/// seed conf, so injecting it is a harmless duplicate.
pub(crate) fn loop_iteration_confs(flow: &Flow) -> Vec<NodeConf> {
    let mut out = Vec::new();
    for (ep_id, table) in &flow.loops {
        let Some(fg) = flow.entrypoints.get(ep_id) else {
            continue;
        };
