//! The DSP domain's `Ref` extension, the per-reference `inline` flag, and the
//! data-level DSP-graph discovery backing its inspector toggle.
//!
//! [`DspRefExt`] is stored in the reference node's ext slot under
//! [`DSP_REF_EXT_KEY`], see [`Ref::set_ext`](gantz_core::node::Ref::set_ext).
//! It is stored only when non-default so a default-configured reference keeps
//! its address. `crate::egui`'s `DspRefExtUi` renders the inspector toggle for
//! references whose graph contains DSP nodes, behind the `egui` feature.
...
/// [`node_dsp_of`](crate::node_dsp_of) matches is unconditionally DSP.
/// Sourced from the types' [`NodeTag`] consts so a renamed tag cannot drift.
/// Keep in step with [`node_dsp_of`](crate::node_dsp_of).
const DSP_NODE_TAGS: [&str; 10] = [
    crate::Buffer::TAG,
    crate::Bus::TAG,
    crate::Envgen::TAG,
    crate::Out::TAG,
...
///
/// A literal rather than `NamedRef::TAG` so the headless build, with no
/// `gantz_egui` in the tree, still recognises stored references. The tag is
/// already a cross-crate wire convention, since `gantz_format`'s raise and
/// lower match it at the data level. The `egui`-gated test below pins this
/// constant to the type's own `NodeTag` so a renamed tag cannot drift
/// silently.
...
/// graph it references, so DSP-ness flows through it. `FnNamedRef` is absent
/// by design. A function value references a graph without standing in for
/// it, mirroring its missing `AsRefNode` impl.
const REF_NODE_TAGS: [&str; 2] = [gantz_core::node::Ref::TAG, NAMED_REF_TAG];

/// The graph addresses of every registry graph containing DSP nodes, directly
/// or transitively through references. They are [`ContentAddr`]s, the form
/// [`Ref::content_addr`](gantz_core::node::Ref::content_addr) reports.
///
/// A pure walk over the stored [`DataGraph`](gantz_ca::DataGraph)s. DSP nodes
/// are recognised by wire tag. References are followed through their
/// [`NodeData`](gantz_ca::NodeData) `refs` column, since a reference node's
/// one ref is its target per `Node::required_addrs`. No typed node set or
