...
## [0.7.1](https://github.com/nannou-org/gantz/compare/gantz_egui-v0.7.0...gantz_egui-v0.7.1) - 2026-09-26

### Other

- updated the following local packages: gantz_core, gantz_ui, gantz_format, gantz_std
...
- *(gantz_egui)* keep stacked plot rows within the given height
...
- *(gantz_egui)* do not link the private PlotParams from public docs
- use run_ui in headless egui tests
- *(gantz_egui)* return the plot response from show_plot and share stacked rows
- *(gantz_egui)* share plot look, body and renderer helpers
...
- *(egui,gantz)* base source selector in the graph config pane
- *(egui)* cross-source base refs
- *(format)* seeded name resolution for ref lowering
- *(egui)* RefExtUi inspector seam
- *(egui)* ref ext integration
- *(egui)* domain-extensible settings subtabs
- *(std,egui,bevy)* export per-domain builtins() lists
- persist pop-out window size across sessions
- *(gantz_egui)* host surface for native pane windows (#271, part 1)
- *(gantz_egui)* pop panes out into windows (#271, part 1)
- *(gantz_plyphon)* multichannel signals as channel-group edges
- *(gantz_egui)* plot accepts Steel vectors + backs the scope with VectorV
- *(gantz_egui)* plot a list-of-lists as stacked per-channel sub-plots
- *(gantz_egui)* collapse the inspector's default state row to one line
...
- *(gantz_egui,bevy_gantz_egui)* carry live layout forward on viewless navigation
- *(gantz_egui)* plot size gets the same interaction-only discipline
- *(gantz_egui)* comment size is written only by genuine interaction
...
- *(gantz_egui)* size_sync begin/store absorb the shared frame plumbing
- *(bevy)* move DSP settings onto the domain seam
- fix private intra-doc links so `cargo doc` passes
- *(gantz_egui)* fix stale OpenNodeView "Node Views pane" reference
- *(gantz_egui)* extract render_pane + pane_title from TreeBehaviour
