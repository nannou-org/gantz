...
///
/// Records VM execution timing if `PerfVm` resource is present.
pub fn on_eval_event(
    trigger: On<EvalEvent>,
    ...
    if let Some(vm) = vms.get_mut(&event.head) {
        ...
        if let Some(ref mut perf) = perf_vm {
            perf.0.record(start.elapsed());
        }
    }
}
