...
///
/// Declared alongside the node type itself so that every application
/// composing the node set agrees on the same wire format. Usually
/// implemented via the derive of the same name, which defaults the tag to
/// the type's name and takes a `#[tag("...")]` override:
///
/// ```
